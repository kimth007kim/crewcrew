window.addEventListener("DOMContentLoaded", function(){
    ...
    let FilterOffset = window.pageYOffset + FilterWrap.getBoundingClientRect().top;
    window.addEventListener("resize", function(){
        FilterOffset = window.pageYOffset + FilterWrap.getBoundingClientRect().top;
    });
    window.addEventListener("scroll", function(){ //스크롤 시 필터 sticky기능
        if(window.innerWidth>=768){
            ...
            if(window.scrollY >= FilterOffset - 60){
                FilterWrap.classList.add("Fixed");
            } else {
                FilterWrap.classList.remove("Fixed");
            }
