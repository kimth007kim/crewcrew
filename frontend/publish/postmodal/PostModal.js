window.addEventListener("DOMContentLoaded", function(){
    ...
    loginInput.forEach((e) => {
        ...
        e.addEventListener('focus', function () {
            ...
            if (this.value) { //인풋값이 있을 때 지우기 버튼 show
        ...
        e.addEventListener('blur', function () {
            ...
            if (!this.value) { //인풋값이 없을 때 지우기 버튼 hide
    ...
    Input.forEach((e) => { //다른 인풋 focus시에 드랍다운 올라가게
    ...
    Choose.forEach((e) => {
        e.addEventListener('focus', function(){ //focus시 드랍다운
    ...
    function thisListUp(){ //value값 유무에 따라 드랍다운, 업
        const thisList = this.closest(".PostListDrop");
        if (this.value) {
    ...
    Radio.forEach((e) => { //상세카테고리 하위 메뉴 클릭 시
    ...
    ArrowNum.forEach((e) => { //모집인원 증가/감소 화살표 클릭 시
        e.addEventListener("click", function(){
